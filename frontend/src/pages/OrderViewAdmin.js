...
import { orderDeliver, resetOrderDeliver } from '../features/admin/adminSlice';
...
function OrderViewAdmin() {
  ...
  const { orderDeliverLoading, orderDeliverSuccess, orderDeliverError } =
  ...
  }, [orderDeliverSuccess, dispatch, orderId]);
          ...
          <Card>
            <ListGroup variant="flush">
              ...
              <ListGroup.Item>
                <Row>
                  <Col>Items Price</Col>
                  <Col>€{addDecimals(totalPrice - taxPrice)}</Col>
                </Row>
              </ListGroup.Item>
              <ListGroup.Item>
                <Row>
              ...
              <ListGroup.Item>
                {!isDelivered && (
                  <Button
                    onClick={() => dispatch(orderDeliver(_id))}
                    className="w-100"
                  >
                    Mark As{!isPaid && ' Paid and'} Delivered
                  </Button>
                )}
                {orderDeliverLoading && <Loader />}
                {orderDeliverError !== '' && (
                  <Alert variant="danger">{orderDeliverError}</Alert>
                )}
              </ListGroup.Item>
            </ListGroup>
          </Card>
